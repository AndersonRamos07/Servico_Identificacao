...
{
    public class ComandoEditarEmpresa : Notifiable, ICommand
    {
        public ComandoEditarEmpresa() { }

        public ComandoEditarEmpresa(int codigoRegimeTributavel, int id, string cnpj, string razaoSocial, string telefone, string cep, string logradouro, string numero, string complemento, string bairro, string codCidade, string cidade, string uf, string inscricaoEstadual)
        {
        ...
        private void ConfiguracaoDosTamanhosDosCamposRecebidos()
        {
            AddNotifications(new Contract()
                ...
                .HasMaxLen(InscricaoEstadual, 15, "Inscrição Estadual", "A inscrição estadual informada deve conter no maximo de 15 caracteres"));
        }
